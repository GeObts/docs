...
4. **0% operator fee** on block rewards. The pool earns from Hashpower NFT sales, not from your mining.
5. Payouts go out automatically. There is no "claim" button anywhere.
...
<CardGroup cols={3}>
  <Card title="Home miners" icon="hard-drive">
    Anyone running an ASIC — Apollo, Bitaxe, S19, S21, Whatsminer — pointing at our stratum endpoint.
  </Card>
...
<Steps>
  ...
  <Step title="The remainder is distributed">
    The pool wallet receives the ~2.1+ BTC remainder on-chain. The operator broadcasts a single Bitcoin transaction splitting it among contributors proportional to their work.
  </Step>
...
<Warning>
There is no "claim" button anywhere on BasedMining.
</Warning>
